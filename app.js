...
function setAnalyticsPeriod(days, btn) {
  ...
  if (label) label.textContent = days === 7 ? 'Last 7 days' : days === 90 ? 'Last 90 days' : 'Last 30 days';
...
function renderAnalytics() {
  if (!document.getElementById('ac-posts')) return;
  const igConnected = !!localStorage.getItem('ig_access_token');
  const ttConnected = !!localStorage.getItem('tt_access_token');
  const period = _analyticsPeriod;

  const mockPosts = period === 7 ? 8 : period === 90 ? 42 : 18;
  const mockLikes = period === 7 ? 1240 : period === 90 ? 15800 : 4320;
  const mockFollowers = (igConnected || ttConnected) ? (period === 7 ? 2847 : period === 90 ? 3140 : 2990) : 0;
  const mockReach = period === 7 ? 8400 : period === 90 ? 62000 : 21500;

  function fmt(n) { return n >= 1000 ? (n/1000).toFixed(1)+'K' : n.toString(); }

  document.getElementById('ac-posts').textContent = mockPosts;
  document.getElementById('ac-likes').textContent = fmt(mockLikes);
  document.getElementById('ac-followers').textContent = (igConnected || ttConnected) ? fmt(mockFollowers) : '--';
  document.getElementById('ac-reach').textContent = fmt(mockReach);
  document.getElementById('ac-posts-d').textContent = '↑ 12% vs prev';
  document.getElementById('ac-likes-d').textContent = '↑ 8% vs prev';
  document.getElementById('ac-followers-d').textContent = (igConnected || ttConnected) ? '↑ 3.2% vs prev' : 'Connect an account';
  document.getElementById('ac-reach-d').textContent = '↑ 22% vs prev';

  document.getElementById('ig-avg-likes').textContent = igConnected ? '214' : '--';
  document.getElementById('ig-avg-comments').textContent = igConnected ? '18' : '--';
  document.getElementById('ig-avg-reach').textContent = igConnected ? '1.2K' : '--';
  document.getElementById('ig-best-day').textContent = igConnected ? 'Tuesday' : '--';
  document.getElementById('tt-avg-likes').textContent = ttConnected ? '842' : '--';
  document.getElementById('tt-avg-comments').textContent = ttConnected ? '64' : '--';
  document.getElementById('tt-avg-views').textContent = ttConnected ? '8.4K' : '--';
  document.getElementById('tt-best-day').textContent = ttConnected ? 'Friday' : '--';
  ...
  const pts = period > 30 ? 12 : period;
  const days = Array.from({length: pts}, (_, i) => {
    const d = new Date();
    d.setDate(d.getDate() - Math.round((period / pts) * (pts - 1 - i)));
    return d.toLocaleDateString('en-US', {month:'short', day:'numeric'});
  });
  const igGrowth = days.map((_, i) => 1800 + Math.round(i * 28 + (i*7)%40));
  const ttGrowth = days.map((_, i) => 980 + Math.round(i * 18 + (i*5)%30));

  const fCtx = document.getElementById('chart-followers');
  if (fCtx) {
    _analyticsCharts.followers = new Chart(fCtx.getContext('2d'), {
      type: 'line',
      data: {
        labels: days,
        datasets: [
          { label: 'Instagram', data: igConnected ? igGrowth : [], borderColor: '#E1306C', backgroundColor: 'rgba(225,48,108,0.08)', tension: 0.4, pointRadius: 2, borderWidth: 2, fill: true },
          { label: 'TikTok', data: ttConnected ? ttGrowth : [], borderColor: '#444', backgroundColor: 'rgba(0,0,0,0.04)', tension: 0.4, pointRadius: 2, borderWidth: 2, fill: true }
        ]
      },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { font: { size: 9 }, maxRotation: 0 }, grid: { display: false } }, y: { ticks: { font: { size: 9 } }, grid: { color: 'rgba(0,0,0,0.04)' } } } }
    });
  }

  const eCtx = document.getElementById('chart-engagement');
  if (eCtx) {
    const postLabels = Array.from({length: Math.min(mockPosts, 8)}, (_, i) => 'Post '+(i+1));
    const seed = [312,180,440,95,380,210,520,145];
    _analyticsCharts.engagement = new Chart(eCtx.getContext('2d'), {
      type: 'bar',
      data: { labels: postLabels, datasets: [{ label: 'Engagements', data: seed.slice(0, postLabels.length), backgroundColor: 'rgba(124,63,204,0.75)', borderRadius: 6, borderSkipped: false }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { font: { size: 9 } }, grid: { display: false } }, y: { ticks: { font: { size: 9 } }, grid: { color: 'rgba(0,0,0,0.04)' } } } }
    });
  }
...
function renderHeatmap() {
  ...
  const scores = [
    [0.3,0.5,0.4,0.6,0.7,0.3,0.2],
    [0.5,0.7,0.6,0.8,0.9,0.4,0.3],
    [0.8,0.9,0.8,1.0,0.9,0.5,0.4],
    [0.6,0.8,0.7,0.9,0.8,0.6,0.5],
    [0.9,0.8,0.9,0.8,1.0,0.7,0.6],
    [0.5,0.6,0.5,0.6,0.7,0.8,0.7]
  ];
  const p = (o) => 'rgba(124,63,204,' + o.toFixed(2) + ')';
  let html = '<div class="heatmap-grid">';
  html += '<div></div>';
  dayLabels.forEach(d => { html += '<div class="heatmap-day-label">' + d + '</div>'; });
  hours.forEach((h, hi) => {
    const lbl = h >= 12 ? (h===12?'12pm':(h-12)+'pm') : h+'am';
    html += '<div class="heatmap-hour-label">' + lbl + '</div>';
    dayLabels.forEach((_, di) => {
      const s = scores[hi][di];
      html += '<div class="heatmap-cell" style="background:' + p(0.1 + s*0.9) + '" title="' + (s>0.7?'Great time':s>0.4?'Good time':'Low traffic') + '"></div>';
    });
  });
  html += '</div>';
...
// Wrap switchTab to trigger analytics render
(function() {
